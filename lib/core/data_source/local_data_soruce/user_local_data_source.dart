...
import 'package:hotelapp/core/data_source/local_data_soruce/cache_exception.dart';
...
abstract class UserLocalDataSource {
  ...
  Future deleteOldProfileImage(String imagePath);
...
class UserLocalDataSourceImp extends UserLocalDataSource {
  ...
  Future<bool> hasValidToken() async {
    try {
      final user = userBox.get(_currentUserKey);
      return user != null && user.loginToken!.isNotEmpty;
    } catch (e) {
      return false;
    }
  }
  ...
  Future<void> updateProfileImage(String imagePath) async {
    try {
      ...
      await userBox.put(_currentUserKey, updatedUser);
      await userBox.flush();
...
Future<void> logout() async {
  try {
    final user = await getCurrentUser();
    final updatedUser = user.copyWith(loginToken: null);
    await userBox.put(_currentUserKey, updatedUser);
    await userBox.flush();
  } catch (e) {
